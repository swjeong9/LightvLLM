...
class TestRoPEWrappers:
    """kernels.pos_encoding 래퍼와 layers.rotary_embedding nn.Module 검증

    _C 직접 호출 결과와 비교하여, Python 래퍼 계층이
    인자 전달을 올바르게 수행하는지 확인합니다.
    """

    def test_rotary_embedding_wrapper(self):
        """
        kernels.pos_encoding.rotary_embedding() 래퍼 검증

        래퍼가 _C.rotary_embedding()과 동일한 in-place 결과를 생성하는지 확인
        """
        num_tokens = 32
        num_heads = 8
        num_kv_heads = 2
        ...
        # _C 직접 호출
        query_direct = query.clone()
        key_direct = key.clone()
        _C.rotary_embedding(positions, query_direct, key_direct,
                            head_size, cos_sin_cache, True)

        # 래퍼 호출
        query_wrapper = query.clone()
        key_wrapper = key.clone()
        rotary_embedding(positions, query_wrapper, key_wrapper,
                         head_size, cos_sin_cache, True)

        torch.testing.assert_close(query_wrapper, query_direct, atol=0, rtol=0)
        torch.testing.assert_close(key_wrapper, key_direct, atol=0, rtol=0)

    def test_rotary_embedding_module(self):
        """
        layers.rotary_embedding.RotaryEmbedding nn.Module 검증

        Module이 내부 cos/sin 캐시를 올바르게 생성하고,
        forward()가 커널과 동일한 결과를 반환하는지 검증합니다.
        """
        num_tokens = 32
        num_heads = 8
        num_kv_heads = 2
        ...
        # 동일한 캐시로 _C 직접 호출
        cos_sin_cache = module.cos_sin_cache
        query_direct = query.clone()
        key_direct = key.clone()
        _C.rotary_embedding(positions, query_direct, key_direct,
                            head_size, cos_sin_cache, True)
        ...
        # Module forward
        query_module = query.clone()
        key_module = key.clone()
        module(positions, query_module, key_module)

        torch.testing.assert_close(query_module, query_direct, atol=0, rtol=0)
        torch.testing.assert_close(key_module, key_direct, atol=0, rtol=0)

    def test_module_cache_correctness(self):
        """
        RotaryEmbedding 내부 캐시가 테스트의 generate_cos_sin_cache()와 동일한지 확인
        """
        head_size = 64
        max_position = 128
        ...
        expected_cache = generate_cos_sin_cache(max_position, head_size)

        torch.testing.assert_close(
            module.cos_sin_cache, expected_cache, atol=0, rtol=0
        )

    def test_module_position_zero_identity(self):
        """
        RotaryEmbedding Module에서 position=0일 때 항등 변환 확인
        """
        num_tokens = 8
        num_heads = 4
        num_kv_heads = 2
