...
## ✅ 완료 — Session 2: LlamaMLP

**파일: `lightvllm/layers/mlp.py`** (97줄)
...
- `act_fn`: `SiluAndMul()` (기존 CUDA 커널 재사용)

**테스트 (`tests/layers/test_mlp.py`)** — 7개 테스트:
- output shape, fused vs separate 수치 비교, dtype (fp16/bf16), LLaMA-3 8B 크기 (4096, 14336), zero 입력, weight_loader 로딩
- 전체 90개 테스트 통과 (회귀 없음)
