...
(상세 내용은 `2026-02-24_linear_mlp.md` 참조)

- **Linear 레이어** — `lightvllm/layers/linear.py` (175줄)
  - `Linear`: F.linear 기반 단순 선형 레이어 (cuBLAS GEMM 활용)
  - `MergedLinear`: 여러 프로젝션을 하나의 GEMM으로 융합 (gate+up, Q+K+V)
  - `weight_loader`: HF 체크포인트의 별도 가중치를 융합 파라미터에 shard 단위 로딩
  - `SHARD_GATE`, `SHARD_UP` 상수로 MLP shard ID 가독성 확보
  - 왜 CUDA 커널이 없는지에 대한 교육 주석 (compute-bound vs memory-bound)
- **LlamaMLP** — `lightvllm/models/llama.py` (107줄)
  - SwiGLU 구조: gate_up_proj(MergedLinear) → SiluAndMul(CUDA 커널) → down_proj(Linear)
  - 기존 Linear, MergedLinear, SiluAndMul 모듈 재사용
- **Python 테스트** — `tests/layers/test_linear.py` (20개), `tests/models/test_llama.py` (7개 MLP + 2개 스텁)
- **상태**: Linear 학습 완료, **MLP 학습 필요**

### 2026-03-06 — Attention Backend + KV Cache 코드 작성

(상세 내용은 `2026-03-06_attention_backends.md` 참조)

- **Attention Backend 인프라** 코드 작성 완료:
  - `lightvllm/attention/backends/base.py` (62줄) — AttentionBackend ABC
  - `lightvllm/attention/backends/naive.py` (121줄) — 수동 scaled dot-product attention + GQA
  - `lightvllm/attention/backends/sdpa.py` (83줄) — F.scaled_dot_product_attention 래퍼
  - `lightvllm/attention/kv_cache.py` (163줄) — Contiguous KV cache 버퍼
  - `tests/attention/test_attention.py` (611줄) — 포괄적 테스트 스위트
- **LlamaAttention** — `lightvllm/attention/layer.py` 는 빈 스텁 (미구현)
- **상태**: 코드 작성 완료, **학습 및 테스트 검증 필요**

### 2026-03-06 — LlamaAttention → LlamaForCausalLM 완성 + HF 검증

(상세 내용은 `2026-03-06_llama_model.md` 참조)

- **LlamaAttention** — `lightvllm/models/llama.py`
  - MergedLinear QKV 융합 + RoPE + SDPA + KV Cache 통합
  - 10단계 forward: qkv_proj → split → contiguous → RoPE → reshape → cache → attention → o_proj
- **LlamaDecoderLayer** — Pre-Norm Residual 패턴
  - fused_add_rms_norm으로 덧셈+정규화 1회 커널 호출
  - 첫 번째 layer: residual=None (plain RMSNorm), 이후: fused
- **LlamaModel** — Embedding + DecoderLayer × N + 최종 RMSNorm
  - residual을 layer 간 명시적 전달 (gradient highway)
  - kv_cache.advance()는 모든 layer 후 1회만 호출
- **LlamaForCausalLM** — LM Head + from_pretrained
  - logits.float() (bf16 → fp32 수치 안정성)
  - stacked_params_mapping으로 HF 가중치 융합 로딩
  - tie_word_embeddings 지원
- **HF 검증** — LLaMA-3.2-3B-Instruct (bf16)
  - "The capital of France is" → 양쪽 모두 " Paris" 예측 (argmax 일치)
  - max abs diff ~0.125 (bf16 28-layer 누적 정상 범위)
- 단위 테스트 27개 + HF 검증 1개, **전체 28개 통과**
- **상태**: Phase 1 추론 파이프라인 완성
...
| Linear (Linear, MergedLinear) | ✅ 완료 | `lightvllm/layers/linear.py` |
| LlamaMLP (SwiGLU FFN) | ✅ 완료 | `lightvllm/models/llama.py` |
| Attention Backends + KV Cache | ✅ 완료 | `lightvllm/attention/` |
| LlamaAttention (GQA + RoPE + KV Cache) | ✅ 완료 | `lightvllm/models/llama.py` |
| LlamaDecoderLayer (Pre-Norm Residual) | ✅ 완료 | `lightvllm/models/llama.py` |
| LlamaModel (Embedding + Layers + Norm) | ✅ 완료 | `lightvllm/models/llama.py` |
| LlamaForCausalLM (LM Head + from_pretrained) | ✅ 완료 | `lightvllm/models/llama.py` |
| Weight Loader (safetensors) | ✅ 완료 | `lightvllm/models/loader.py` |
| HF 검증 (LLaMA-3.2-3B-Instruct) | ✅ 완료 | `tests/models/test_llama.py` |

---

## 다음 예정 작업 — Phase 2

Phase 1 완료. Phase 2에서는 vLLM의 핵심 최적화 기술을 구현한다:

1. **PagedAttention** — 고정 크기 블록 단위 KV 캐시 동적 할당 (메모리 단편화 해결)
2. **FlashAttention** — O(N) 메모리 attention (tiling + online softmax)
3. **Continuous Batching** — 요청 단위 동적 배칭 (throughput 극대화)
4. **범용 Attention 레이어** — `lightvllm/attention/layer.py` (backend 추상화)
