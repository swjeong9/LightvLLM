...
(본 세션에서 수행, 상세 내용은 `2026-02-11_rmsnorm.md` 참조)
...
| Python 커널 래퍼 | ✅ 완료 (RoPE, RMSNorm) | `lightvllm/kernels/` |
| Python 레이어 | ✅ 완료 (RMSNorm) | `lightvllm/layers/normalization.py` |
| Activation 커널 (SiLU, SiLU+Mul) | ⏳ 미완 | `csrc/activation_kernels.cu` |
...
1. **Activation 커널** — SiLU, SiLU+Mul fused (LLaMA MLP의 gate_proj 패턴)
2. **레이어 구현** — Linear, MLP, RotaryEmbedding Python 레이어
3. **Attention** — naive Self-Attention 구현
4. **LLaMA 모델 조립** — 전체 파이프라인 연결 + HuggingFace 검증
